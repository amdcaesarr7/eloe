...
async function saveDirHandle(handle) {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open('eloe-db', 1);
...
// Try to auto-load saved folder on startup
async function tryAutoLoad() {
  ...
  try {
    ...
    if (perm === 'granted') {
      dirHandleGlobal = handle;
      btnLoad.textContent = '🔄 ' + handle.name;
      await indexDatabase(handle);
    } else {
      // Permission needs re-granting, just silently skip (user can click button)
    }
  } catch (e) {
...
btnLoad.addEventListener('click', async () => {
  try {
    ...
    btnLoad.textContent = '🔄 ' + dirHandle.name;
...
function loadProfileData(p) {
  ...
  // Deduplicate followingList while preserving original casing for display
  const seenFollowing = new Set();
  const followingList = (p.followingList || []).filter(u => {
  ...
  // Traitors = victim follows them, but they don't follow back
